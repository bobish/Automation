...
CSV.open("#{county}_7.csv", 'w') do |csv|
  # write the headers as the first row
  csv << headers
end

CSV.open("#{county}_7.csv","a+") do |csv|
	...
	con.query("SELECT * FROM `property`").each do |row|
		...
		if(parcel_type == "R" or parcel_type == "M")
			que = "Select * from exemptions where `exemptions`.`OwnerQuickRefID` = '#{cad_propertyid}' "
			exempts = con.query(que)		
			exelist = ""
			over = ""
			if exempts.size > 0
				
				exempts.each do |row|
					ee = row['ExemptionCode']
					if ee.strip == "" and over == ""
						over = "OAS"
					end
					if exelist == ""
						if ee != ""
			...
			if exelist.include?("HS")
				if over != ""
					exelist = exelist+","+over
				end
			end
			
		# puts cad_propertyid, exelist
		exemptions = ""
		if exelist != ""
			ecount = 1
		...
		# puts qu2
			...
			# CSV.open("exempt_qf.csv","a+") do |exe|
				ownres.each do |ow|
