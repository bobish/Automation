<?php
	...
	$headerText = file_get_contents($path."\\".$common."APPR_HDR.txt");


	$appInfoText = file_get_contents($path."\\".$common."PROP.txt");
	$appInfoHandle = fopen($path."\\".$common."PROP.txt", "r");
	...
	if ($appInfoHandle) {
		while (($line = fgets($appInfoHandle)) !== false) {
			...
			if($prop_type_cd != "P")
			{
				...
				$cad_geo_id = trim(substr($line,4434, 25));
